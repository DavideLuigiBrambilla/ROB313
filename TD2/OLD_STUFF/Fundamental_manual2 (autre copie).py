...
for SampleSize in range(N):
	...
	F_7, nb_inliners, pts1_7, pts2_7, inliner_pts1, inliner_pts2 = Run_7points (pts1, pts2, 3)  #POP = 50, STAR = 
	
	## Vérifie à nouveau quelle est la meilleure matrice fondamentale par rapport au nombre d'inliners
	ratio = nb_inliners/shape(pts1)[0]
	if ratio > nb_inliners_best:
		x = []
		x_prime = []
		verif = []
		for i in range(shape(inliner_pts1)[0]):
			a = np.array(inliner_pts1)
			x.append([a[i][0], a[i][1], 1])
			a = np.array(inliner_pts2)
		...
		nb_inliners_best = ratio
		bestF_7 = F_7
		best_pt1 = pts1_7
		best_pt2 = pts2_7
		best_line=SampleSize
		print(">>>>>>>>>>>>>>>>>>>> Number of inliners: ", nb_inliners)
		print (err_mean, err_best)
	print ("+++++++++++++")
