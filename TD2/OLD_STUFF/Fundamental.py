...
for i,(m,n) in enumerate(matches):
    if (m.distance < 0.9) & (m.distance < 0.9*n.distance):  #Par défaut : 0.99. For Lowe's paper, 0.7
...
for SampleSize in range(N):
	...
	for i in range (7):
		sevenPoints.append(random.randrange(0, shape(pts1)[0]))
	
	FRansac, mask = cv2.findFundamentalMat(pts1,pts2,cv2.FM_7POINT)
		
	print("OOO", sevenPoints,shape(pts1),shape(pts2) )
