             ...
             int o_min)
{
  ...
  if (noctaves < 0) {
    noctaves = (int)VL_MAX (floor (log2 (VL_MIN(width, height))) - o_min - 3, 1) ;
  }
...
#define Aat(i,j)     (A[(i)+(j)*3])
    ...
    /* check threshold and other conditions */
    {
      ...
      if (good) {
        ...
        k-> x     = (float)(xn * xper) ;
        k-> y     = (float)(yn * xper) ;
        k-> sigma = (float)(f->sigma0 * pow (2.0, sn/f->S) * xper) ;
        ++ k ;
      }
...
#define SAVE_BACK                                                       \
    ...
    *grad++ = vl_mod_2pi_f   ((float)(vl_fast_atan2_f (gy, gx) + 2*VL_PI)) ;     \
    ++src ;                                                             \
                                    ...
                                    VlSiftKeypoint const *k)
{
  ...
  int          W      = (int)VL_MAX(floor (3.0 * sigmaw), 1) ;
...
#define atd(dbinx,dbiny,dbint) *(dpt + (dbint)*binto + (dbiny)*binyo + (dbinx)*binxo)
  ...
  for(dyi =  VL_MAX(- W,   - yi   ) ;
    ...
    for(dxi =  VL_MAX(- W,   - xi   ) ;
	  ...
	  vl_sift_pix theta = vl_mod_2pi_f(angle - (vl_sift_pix)angle0);

      /* fractional displacement */
      vl_sift_pix dx = (vl_sift_pix)(xi + dxi - x);
      vl_sift_pix dy = (vl_sift_pix)(yi + dyi - y);

      /* get the displacement normalized w.r.t. the keypoint
         orientation and extension */
      vl_sift_pix nx = (vl_sift_pix)(( ct0 * dx + st0 * dy) / SBP) ;
      vl_sift_pix ny = (vl_sift_pix)((-st0 * dx + ct0 * dy) / SBP) ;
      vl_sift_pix nt = (vl_sift_pix)(NBO * theta / (2 * VL_PI)) ;

      /* Get the Gaussian weight of the sample. The Gaussian window
       * has a standard deviation equal to NBP/2. Note that dx and dy
       * are in the normalized frame, so that -NBP/2 <= dx <=
       * NBP/2. */
      vl_sift_pix const wsigma = (vl_sift_pix)f->windowSize ;
      vl_sift_pix win = (vl_sift_pix)fast_expn
        ((nx*nx + ny*ny)/(2.0 * wsigma * wsigma)) ;

      /* The sample will be distributed in 8 adjacent bins.
         We start from the ``lower-left'' bin. */
      int         binx = vl_floor_f (nx - 0.5f) ;
      int         biny = vl_floor_f (ny - 0.5f) ;
      int         bint = vl_floor_f (nt) ;
      vl_sift_pix rbinx = (vl_sift_pix)(nx - (binx + 0.5)) ;
      vl_sift_pix rbiny = (vl_sift_pix)(ny - (biny + 0.5)) ;
      vl_sift_pix rbint = (vl_sift_pix)(nt - bint) ;
      int         dbinx ;
      int         dbiny ;
      int         dbint ;
